...
from flask import Flask, flash, render_template, request, url_for, redirect,session,redirect
import hashlib
...
sheet.write(0, 0, 'Original Password')
sheet.write(0, 1, 'Hashed Password')

# Write the data
password = "mysecretpassword"
hashed_password = generate_password_hash(password)[:3]
sheet.write(1, 0, password)
sheet.write(1, 1, hashed_password)
...
def pack(pid,name,desc,price,mname,hash1):
    buffer.append(pid)
...
def login():
    if request.method == 'POST':
        ...
        if username == default_username and check_password_hash(generate_password_hash(default_password), password):
...
def listmethod():
        if(request.method=='POST'):
            ...
            result=add(ID,name,desc,price,mname,hash1)
            if(result==True):
